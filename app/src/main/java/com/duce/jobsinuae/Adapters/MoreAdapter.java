...
public class MoreAdapter extends RecyclerView.Adapter<MoreAdapter.ViewHolder> {
    ...
    public MoreAdapter(ArrayList<Posts> posts , Context context) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if(postsArrayList.get(position).getEmbedded().getWpFeaturedmedia().size() == 0){
            ...
            Picasso.get()
                    .load(
                            postsArrayList.get(position).getEmbedded().getWpFeaturedmedia().get(0).getSourceUrl().toString() == " "
                            ? "https://image.freepik.com/free-vector/error-404-concept-illustration_114360-1811.jpg"
                                    : postsArrayList.get(position).getEmbedded().getWpFeaturedmedia().get(0).getSourceUrl().toString()
                    )
                    .placeholder(R.drawable.splashicon)
                    .error(R.drawable.splashicon)
